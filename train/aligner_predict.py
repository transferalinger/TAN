"""
File: train/aligner_predict.py
 - Contain source code for encoding difference calculation of transfer normalization model.
"""
import sys
sys.path.append("..")
...
if __name__ == '__main__':
    ...
    matchnet.init_session()
    train_encoding = matchnet.predict_encoder1(train_d)
    train_normalized = matchnet.predict_matchnet(train_d)
    test_encoding = matchnet.predict_encoder1(test_d)
    test_normalized = matchnet.predict_matchnet(test_d)

    train_error = np.linalg.norm(train_encoding - train_normalized, axis=1)
    test_error = np.linalg.norm(test_encoding - test_normalized, axis=1)
    train_rel_error = train_error / np.linalg.norm(train_encoding, axis=1)
    test_rel_error = test_error / np.linalg.norm(test_encoding, axis=1)
