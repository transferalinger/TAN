"""
File: model/v2.py
 - Contain model class for mlp and transfer normalization layer on top of autoencoder.
"""
import random
import json
...
class V2(ModelBase):
    """
    Description: model class for mlp and transfer normalization layer on top of autoencoder.
    """
    ...
    def __init__(self,
        ...
        :param matchnet_path: the path for the pretrained transfer normalization layer
        :param encoder_lr: the learning rate for encoder training
        :param use_pretrained: whether to fix parameters of pretrained mlp
        :param fix_pretrained: whether to initialize from pretrained mlp
        ...
        self.matchnet_path = matchnet_path
        ...
        self.ae_hyper = load_ae_architecture(self.encoder_path, is_ae=True)
        with tf.variable_scope("ae"):
            self.ae_weights = self.load_ae_weights()
        self.encoder = self.create_encoder()

        # create computation graph of transfer normalization layer using tensorflow
        self.mn_hyper = load_ae_architecture(self.matchnet_path, is_ae=False)
        with tf.variable_scope("mn"):
            self.mn_weights = self.load_mn_weights()
        self.matchnet = self.create_matchnet()
    ...
    def load_mn_weights(self):
        ...
        n_layer = len(self.mn_hyper['hidden']) + 1
        weight_prefix = self.matchnet_path + '/weight'
        # encoder
        for i in range(n_layer-1):
            n = i + 1
            all_weights['weights']['w' + str(n)] = tf.Variable(
                np.loadtxt(weight_prefix + '/mn_w' + str(n),
                           dtype=np.float32,
                           delimiter=','))
            all_weights['biases']['b' + str(n)] = tf.Variable(
                np.loadtxt(weight_prefix + '/mn_b' + str(n),
                           dtype=np.float32,
                           delimiter=','))
    ...
    def initialize_weights(self):
        ...
        units = [self.ae_hyper['output']] + self.network_architecture['hidden']
        print(units)
    ...
    def create_matchnet(self):
        ...
        weights = self.mn_weights['weights']
        biases = self.mn_weights['biases']

        layer = self.encoder
        for n in range(1, len(self.mn_hyper['hidden'])+1):
            print(layer)
    ...
    def create_loss_optimizer(self):
        ...
        if self.pretrain_trainable:
            self.ae_optimizer = optimizer(self.encoder_lr).minimize(
                    self.cost,
                    var_list=ae_vars)
            self.mn_optimizer = optimizer(self.encoder_lr).minimize(
                    self.cost,
                    var_list=mn_vars)
        else:
            self.ae_optimizer = None
            self.mn_optimizer = None
    ...
    def dump_weight(self):
        ...
        mn_units = [self.ae_hyper['z']] + self.mn_hyper['hidden']
        m_units = [self.ae_hyper['output']] + self.network_architecture['hidden']
        ...
        # MN hidden layers
        n_layer = len(mn_units)
        for n in range(1, n_layer):
            w = self.mn_weights['weights']['w' + str(n)]
            b = self.mn_weights['biases']['b' + str(n)]
            dumps.append(self.sess.run([w, b]))

        mn_out_w = self.mn_weights['weights']['out']
        mn_out_b = self.mn_weights['biases']['out']
    ...
    def save_weight(self, dumps, _dir):
        ...
        n_layer = len(self.mn_hyper['hidden'])
        for i in range(n_layer):
            n = i+1
    ...
    def learn(self, test_d, test_l, test_step, early_stop, device):
        ...
        try:
            ...
            while not coord.should_stop():
                ...
                if self.pretrain_trainable:
                    (opt_ae, opt_mn, opt_mlp, cost, train_acc) = self.sess.run(
                            [self.ae_optimizer,
                            self.mn_optimizer,
                            self.mlp_optimizer,
                            self.cost,
